...
fn geckolist()-> Result<()> {
    let f = File::open("gecko.txt")?;
    let mut reader = BufReader::new(f);
    let mut buffer = String::new();
    for line in reader.lines() {
	let v: Gecko = serde::Deserialize::deserialize(line);
	println!("{:#?}", v);
    }
    Ok(())
}
...
async fn main() -> Result<()>  {
    // let gport = 8555u16;
    // let ganache = Ganache::new().port(gport).mnemonic("brownies are good").fork("http://localhost:8545").spawn();00
    geckolist();
